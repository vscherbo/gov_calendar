...
loc_res BOOLEAN;
BEGIN
    loc_sql := format('select ARRAY[%s]::text[] <@ regexp_split_to_array(ask_month, %s)::text[] 
    ...
    arg_day, quote_literal(loc_delimiter), quote_literal(loc_year_col), quote_literal(arg_month_name), quote_literal(arg_year));
